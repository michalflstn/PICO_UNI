...
//*****************************************************************
    ...
    if (CONFIG_UPDATE)
    {
      ...
      conf_.HopeDelayFP          =(uint16_t)Vupdateparams[11]; //задержка в первой точке линии   // add 240503  
      ZJump=conf_.HopeZ; // see hopinglin
      flgMaxJump=(ZJump==0);  
      if (flgDebug)
      { 
       sleep_ms(100);   
...
// const int16_t SICMDC=3;      
 ...
 //start
  ...
  if (!flgVirtual)
  {
    ...
    hardware->retract();
    sleep_ms(50);
    deltaZ=DACZMove(0,Z0-abs(ZStart),-10,delay);  //-10  z=0 втянут tested 250423
...
//////////////////////////////////////
 ...
 // else
  dir= 10;  //1
  deltaZ = DACZMove(deltaZ, dlt, dir, MicrostepDelay );// dir
 } //j
 /////////////////////////////////////////  
...
// 250918  
  ...
  if (flgSendString) sendStrData(code+std::to_string(RESONANCE),data,100,true);
                else sendData(40,data,100,true); //250908
   int16_t count = 0;
  while ((!TheadDone) || (count<20) )//ожидание ответа ПК для синхронизации
  {
    sleep_ms(100);
